...
async def command_board(bot, msg, args):
    ...
    if cid in games.keys():
        if games[cid].board is not None:
            ...
            await msg.channel.send("There is no running game in that chat. Please start the game with startgame")
    else:
        await msg.channel.send("There is no game in that chat. Create a new game with newgame")
...
async def command_start(bot, msg, args):
    ...
    await msg.channel.send(
                     ...
                     "the game.\"*\n\nType **newgame** to create a game!", "Introduction"))
...
async def command_newgame(bot, msg, args):
    ...
    if not isinstance(msg.channel, PrivateChannel):
        if cid not in games.keys():
            ...
            await msg.channel.send("There is currently a game running. If you want to end it please type cancelgame!")
    else:
        await msg.author.send("You have to add me to a group first and type newgame there!")
...
async def command_join(bot, msg, args):
    ...
    if not isinstance(msg.channel, PrivateChannel):
        if cid in games.keys():
                ...
                if uid not in game.playerlist:
                    if len(game.playerlist) < 10:
                        ...
                        try:
                            ...
                            await game.channel.send(
                                             fname + ", I can\'t send you a DM. Please try sending join again.")
                    else:
                        await game.channel.send(
            ...
            await msg.channel.send("There is no game in this chat. Create a new game with newgame")
    else:
        await msg.channel.send("You have to add me to a group first and type newgame there!")
        ...
        if game.board is None:
            if msg.author.id == game.initiator or status.administrator:
                ...
                if player_number > 4:
                    ...
                    await game.channel.send("There are not enough players (min. 5, max. 10). Join the game with join")
            else:
                await game.channel.send("Only the initiator of the game or a group admin can start the game with startgame")
        else:
...
async def command_cancelgame(bot, msg, args):
    ...
    if cid in games.keys():
        ...
        if msg.author.id == game.initiator or status.administrator:
            ...
            await msg.channel.send("Only the initiator of the game or a group admin can cancel the game with cancelgame")
    else:
        await msg.channel.send("There is no game in this chat. Create a new game with newgame")
...
async def choose_chancellor(bot, game):
    ...
    await game.board.state.nominated_president.user.send('Please nominate your chancellor with chan <id>!\nYour choices are:\n{}\n\nYou have 30 seconds.'
                                                         .format(
                                                             "\n".join(
    ...
    for uid in game.playerlist:
        ...
        if not game.playerlist[uid].is_dead:
            ...
            await game.playerlist[uid].user.send(
                             "Do you want to elect President %s and Chancellor %s? Say ja or nein to continue. You have 30 seconds." % (
                                 game.board.state.nominated_president.name, game.board.state.nominated_chancellor.name))
    while True:
        def check_vote(msg):
            validation = validate_message(msg)
    ...
    try:
        ...
        if len(game.board.state.drawn_policies) == 3:
            ...
            if answer == "veto":
                ...
                await game.board.state.president.user.send("Say noveto to accept the suggestion. Say veto to reject it. You have 30 seconds.")
...
async def pass_two_policies(bot, game):
    ...
    if game.board.state.fascist_track == 5 and not game.board.state.veto_refused:
        ...
        await game.board.state.chancellor.user.send("President %s gave you the following 2 policies:\n%s\n Enact one of them using enact <id>. You can also use your Veto power: veto. You have 30 seconds." % (game.board.state.president.name, policy_str))
